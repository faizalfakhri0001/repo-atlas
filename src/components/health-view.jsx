  ...
  return (
    <div className="h-full overflow-auto p-5">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <Card><CardHeader><CardTitle>Repository facts</CardTitle><CardDescription>Raw context used by the current rules.</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-3"><Fact label="Commits analyzed" value={facts.processedCommits} /><Fact label="Total commits" value={facts.totalCommits} /><Fact label="Local branches" value={facts.localBranchCount} /><Fact label="Tracked files" value={facts.trackedFileCount} /><Fact label="Large files" value={facts.largeFileCount} /><Fact label="Conflicts" value={facts.conflictedFileCount} /></CardContent></Card>
        <Card><CardHeader><CardTitle>Activity context</CardTitle><CardDescription>Dates and scope help explain what the score covers.</CardDescription></CardHeader><CardContent className="space-y-3 text-xs"><div className="flex justify-between gap-3"><span className="text-muted-foreground">Last commit</span><span title={formatDate(facts.lastCommitAt)}>{facts.lastCommitAt ? formatRelativeDate(facts.lastCommitAt) : "No commit history"}</span></div><div className="flex justify-between gap-3"><span className="text-muted-foreground">Current branch</span><span>{facts.currentBranch || "Detached HEAD"}</span></div><div className="flex justify-between gap-3"><span className="text-muted-foreground">Working changes</span><span>{Number(facts.dirtyFileCount ?? 0).toLocaleString()}</span></div>{scope.sourceTruncated && <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-amber-400">This report is bounded. Commits, branches, or tracked-file metadata were truncated; metrics are not a claim about the entire repository.</div>}</CardContent></Card>
      </div>
    </div>
  );
}
