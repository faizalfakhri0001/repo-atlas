...
function registerIpcHandlers() {
  ...
  const invokeHandlers = {
    ...
    "worktree:remove-preview": async (payload) => previewWorktreeRemove(payload?.repositoryPath, payload?.path, {
      operationMode: await preferences.getOperationMode(),
      currentWorktreePath: payload?.currentWorktreePath,
    }),
    "worktree:remove": (payload) => executeWorkspaceOperation(payload, ({ operationMode }) => removeWorktree(payload?.repositoryPath, payload?.path, {
      operationMode,
      currentWorktreePath: payload?.currentWorktreePath,
    })),
    "worktree:prune-preview": async (payload) => previewWorktreePrune(payload?.repositoryPath, {
      operationMode: await preferences.getOperationMode(),
    }),
    "worktree:prune": (payload) => executeWorkspaceOperation(payload, ({ operationMode }) => pruneWorktrees(payload?.repositoryPath, { operationMode })),
